...
import { getAllOrders, updateOrderStatus, assignAgentToOrder, addOrderNote } from "@/app/actions/orders"
...
import { Search, X, Filter, Facebook, MessageCircle, Globe, CheckCircle2, Clock, Truck, Package, XCircle, Calendar, Download, Check, ChevronUp, ChevronDown, ChevronsUpDown, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function OrdersPage() {
  ...
  const [bulkStatus, setBulkStatus] = useState<OrderStatus>(OrderStatus.CONFIRMED)
  ...
  // Sorting states
  type SortField = "orderNumber" | "customerName" | "totalAmount" | "status" | "createdAt"
  ...
  async function handleBulkAgentAssignment() {
    if (!session?.user?.id || selectedOrderIds.length === 0 || !bulkAgentId) return
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    if (agentFilter !== "all") {
      if (agentFilter === "unassigned" && order.agentId !== null) {
        return false
      } else if (agentFilter !== "unassigned" && order.agentId !== agentFilter) {
        return false
      }
    }
  ...
  useEffect(() => {
    setCurrentPage(1)
  }, [searchQuery, statusFilter, sourceFilter, salesRepFilter, agentFilter, dateFilter])
      ...
      <div className="max-w-[1600px] mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="text-right flex items-center gap-4">
            <div className="text-sm text-gray-600 dark:text-gray-400">
              ...
              {sortedOrders.length !== orders.length && ` (filtered from ${orders.length})`}
            </div>
            <button
              onClick={exportToCSV}
                ...
                {paginatedOrders.length === 0 ? (
                  ...
                  paginatedOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <div className="text-gray-900 dark:text-gray-100">{order.city}</div>
                        <div className="text-gray-500 dark:text-gray-400 text-xs">
                          {order.state}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm">
                        <div className="max-w-xs">
                          {order.items.map((item, idx) => (
                            <div key={item.id} className="text-gray-900 dark:text-gray-100">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white dark:bg-gray-900 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4">
                ...
                {/* Customer Info */}
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-gray-100 mb-2">Customer Information</h4>
                  <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg space-y-2 text-sm">
                    ...
                    <div>
                      ...
                      <span className="text-gray-900 dark:text-gray-100">{selectedOrder.customerPhone}</span>
                    </div>
                    ...
                    <div>
                      ...
                      <span className="text-gray-900 dark:text-gray-100">{selectedOrder.city}</span>, {selectedOrder.state}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg overflow-hidden">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-100 dark:bg-gray-700">
                        <tr>
                          <th className="px-4 py-2 text-left text-gray-700 dark:text-gray-300 font-medium">Product</th>
                          <th className="px-4 py-2 text-right text-gray-700 dark:text-gray-300 font-medium">Qty</th>
                          <th className="px-4 py-2 text-right text-gray-700 dark:text-gray-300 font-medium">Price</th>
                          <th className="px-4 py-2 text-right text-gray-700 dark:text-gray-300 font-medium">Cost</th>
                          <th className="px-4 py-2 text-right text-gray-700 dark:text-gray-300 font-medium">Subtotal</th>
                        </tr>
                      </thead>
                      <tbody>
                      ...
                      <tfoot className="bg-gray-100 dark:bg-gray-700">
                        <tr>
                          <td colSpan={4} className="px-4 py-3 text-right font-semibold text-gray-900 dark:text-gray-100">
                            Total
                          </td>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h4 className="font-medium text-gray-900 dark:text-gray-100 mb-2">Sales Rep</h4>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleAgentAssignment(selectedOrder.id, e.target.value)
                      }
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    >
                ...
                {/* Status Timeline */}
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-gray-100 mb-3">Status Timeline</h4>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-xs text-gray-500 dark:text-gray-400">
                                {formatDateTime(new Date(selectedOrder.confirmedAt))}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-xs text-gray-500 dark:text-gray-400">
                                {formatDateTime(new Date(selectedOrder.dispatchedAt))}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-xs text-gray-500 dark:text-gray-400">
                                {formatDateTime(new Date(selectedOrder.deliveredAt))}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-xs text-gray-500 dark:text-gray-400">
                                {formatDateTime(new Date(selectedOrder.cancelledAt))}
                              </span>
                            </div>
                          </div>
                        </div>
                ...
                {/* Notes & Communication */}
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-gray-100 mb-3">Notes & Communication</h4>
        ...
        {showBulkAgentModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white dark:bg-gray-900 rounded-lg p-6 max-w-md w-full">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">
                Assign Agent to {selectedOrderIds.length} Order{selectedOrderIds.length !== 1 ? "s" : ""}
              </h3>
